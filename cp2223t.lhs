...
\\\hline
e10786 & Ioannis Nezis
\\
e10785 & Kajetan Lösch
\\
a88000 & Gerson Júnior
\\
...
trim' = top 5 . sequence . map (top 5 . filterP.norm)  where
   filterP (D x) = D [(a,p) | (a,p) <- x, p > 0.0001 ]
   top n = vec2Dist . take n . reverse . presort snd . unD 
   vec2Dist x = D [ (a,n/t) | (a,n) <- x ] where t = sum(map snd x) 
\end{code}
...
\subsection*{Problema 1}
We start by deviding the given funktion into auxilary funktions,
which will become the mutual rekursiv funktions.\\
\[f\ a\ b\ c\ (n+3) = a\cdot \underbrace{f\ a\ b\ c\ (n+2)}_{g\ a\ b\ c\ n} + b\cdot \underbrace{f\ a\ b\ c\ (n+1)}_{h\ a\ b\ c\ n}+c \cdot f\ a\ b\ c\ n\]
Now we look at each funktion seperatly:\\
\begin{align*}
g\ a\ b\ c\ 0 &= f\ a\ b\ c\ 2 = 1\\
g\ a\ b\ c\ (n+1) &= f\ a\ b\ c\ (n+3) = a\cdot g\ a\ b\ c\ n  + b\cdot h\ a\ b\ c\ n + c \cdot f\ a\ b\ c\ n \\
h\ a\ b\ c\ 0 &= f\ a\ b\ c\ 1 = 1\\
h\ a\ b\ c\ (n+1) &= f\ a\ b\ c\ (n+2) = g\ a\ b\ c\ n\\
f\ a\ b\ c\ 0 &= 0\\
f\ a\ b\ c\ (n+1) &= h\ a\ b\ c\ (n)
\end{align*}
From here we can derive the pointwise definition of each funktion:\\
\[
\begin{cases}
      g\ a\ b\ c\ \cdot [\underline 0, suc] =& [\underline 1, add \cdot (add \cdot ((a\cdot)\times(b\cdot))) \times (c\cdot))\cdot \langle \langle g,h \rangle, f \rangle] \\
      h\ a\ b\ c\ \cdot [\underline 0, suc] =& [\underline 1, \pi_1\cdot \pi_1 \cdot  \langle \langle g,h \rangle, f \rangle] \\
    f\ a\ b\ c\ \cdot[\underline 0, suc] =& [\underline 0, \pi_2\cdot\pi_1 \cdot  \langle \langle g,h \rangle, f \rangle]
\end{cases}
\]
Using the +-Absorbtion, and the definitions of in Funktion and Funktor we can simplify to:\\
\[
\begin{cases}
      g\ a\ b\ c\ \cdot [\underline 0, suc] =& [\underline 1, add \cdot (add \cdot ((a\cdot)\times(b\cdot))) \times (c\cdot))]\cdot \textbf F \langle \langle g,h \rangle, f \rangle \\
      h\ a\ b\ c\ \cdot [\underline 0, suc] =& [\underline 1, \pi_1\cdot \pi_1] \cdot  \textbf F  \langle \langle g,h \rangle, f \rangle \\
    f\ a\ b\ c\ \cdot[\underline 0, suc] =& [\underline 0, \pi_2\cdot\pi_1] \cdot  \textbf F  \langle \langle g,h \rangle, f \rangle
\end{cases}
\]
Now we can use the more general mutual rekursion law we proved in Ficha 8 to be true.\\
Funções auxiliares pedidas:
\[\langle\langle g, h\rangle, f \rangle = (||\langle\langle [\underline 1, add \cdot (add \cdot ((a\cdot)\times(b\cdot))) \times (c\cdot))], [\underline 1, \pi_1\cdot \pi_1]\rangle, [\underline 0, \pi_2\cdot\pi_1] \rangle  ||)\]
By using the mutual recursion law twise we arive at the following definition:
\[\langle\langle g, h\rangle, f \rangle = (||[\langle\langle \underline 1, \underline 1 \rangle \underline 0 \rangle, \langle \langle add \cdot (add \cdot ((a\cdot)\times(b\cdot))) \times (c\cdot)), \pi_1\cdot \pi_1 \rangle, \pi_2\cdot\pi_1\rangle]||)\]
Therefor we get the following implementation:
\begin{code}
...
Gene de |tax|:
\begin{code}
splitIndent :: Int -> [String] -> [[String]]
splitIndent x strs = tail (aux strs [])
    where
        aux :: [String] -> [String] -> [[String]]
        aux [] acc = [acc]
        aux (h:t) acc | head unindent != ' ' = acc:(aux t [unindent])
                      | otherwise            = aux t (acc ++ [unindent])
            where
                unindent = drop x h

gene = (id -|- (id >< (splitIndent 4))) . out


foo :: [String] -> ([String],[String])
foo = split (takeWhile (\x -> x!!4 == ' ')) (dropWhile (\x -> x!!4 == ' '))

baz = anaList ((id -|- (split ((map (drop 4)).cons . (id ><p1)) (p2.p2))).(id -|- (id >< foo)).outList)
\end{code}
\begin{minipage}[t]{0.6\textwidth}
After we run the out funktion we have either a String or a List of Strings (head and tail).\\
To prepare the recursiv call of the anamorthism, we have to seperate the tail into Lists of Lists.\\
Each List will become a Tree. Further we have to remove the indentation.
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{eqnarray*}
\xymatrix@@C=3cm{
    S + S \times S^*
           \ar[r]_-{id + id \times (splitIndent\ 4)}
&
    S + S \times (S^*)^*
}
\end{eqnarray*}
...
\end{minipage}\\
The main Idea in the splitIndent funktion is to collect Strings while they are deeper indented then the first Line, and therefor a subcategory. As soon as a String has the same indentation, we close the accumulation list and start a new one.\vspace{1cm}\\
...
post = cataExp ((either singl cons) . (singl -|- ((split (singl.p1) (uncurry attachAll)) . (id >< concat))))
\end{code}
\newline
\begin{minipage}[t]{0.6\textwidth}
So here is how this soulution works:
Afther we apply the funktion rekursivly in the Funktor, we get  A List of Lists of Lists of Strings.\\
We Flatten those to just Lists of Lists of strings with concat (the unit funktion of the List Monad).\\
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{eqnarray*}
\xymatrix@@C=2cm{
    S + S \times ((S^*)^*)^*
           \ar[r]_-{id + id \times concat}
&
    S + S \times (S^*)^*
}
\end{eqnarray*}
\end{minipage}
\begin{minipage}[t]{0.6\textwidth}
Then we attach the String of the current Node to every line of the Table with the attachAll funktion.
We want to Maintain the String of the Node, hence we use a split.
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{eqnarray*}
\xymatrix@@C=2cm{
    S + S \times (S^*)^*
           \ar[r]_-{id + \langle \pi_1,\uncurry{attachAll}\rangle}
&
    S + S \times (S^*)^*
}
\end{eqnarray*}
...
\vspace{0.5cm}\\
\begin{minipage}[t]{0.6\textwidth}
Since the result of the catamorphism is a List of Lists of Strings, we now build the abstract type of Lists of Lists:
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{eqnarray*}
\xymatrix@@C=2cm{
    S + S \times (S^*)^*
           \ar[r]_-{signl + singl \times id}
&
    S^* + S^* \times (S^*)^*
}
\end{eqnarray*}
...
\vspace{0.5cm}\\
\begin{minipage}[t]{0.6\textwidth}
Now we just have to use the in funktion to get our result.
\end{minipage}
\begin{minipage}[t]{0.4\textwidth}
\begin{eqnarray*}
\xymatrix@@C=2cm{
...
\end{eqnarray*}
\end{minipage}
Here is a diagram for this catamorphism:\\
\begin{tikzcd}
  {Exp\ S\ S} &&&& {S + S \times (Exp\ S\ S)^*} \\
  \\
  &&&&&& {} \\
  \\
  {(S^*)^*} &&&& {S+S\times((S^*)^*)^*} \\
  ...
  &&&&&& {} \\
  &&& {S+S\times(S^*)^*} \\
  &&&& {attachAll\ a= map (cons\ a)}
  \arrow["out", curve={height=-18pt}, from=1-1, to=1-5]
  \arrow["in", curve={height=-18pt}, from=1-5, to=1-1]
  \arrow["{in=[singl, cons]}", from=7-1, to=5-1]
  \arrow["{\textbf{F}\ post = id + id \times post^*}", from=1-5, to=5-5]
  \arrow["{post = (|gene|)}"', from=1-1, to=5-1]
  \arrow["gene", from=5-5, to=5-1]
  \arrow["{id + id \times concat}", from=5-5, to=7-5]
  \arrow["{id + \langle \pi_1, \uncurry{attachAll}\rangle}", from=7-5, to=9-4]
  \arrow["{singl + singl\times id}", from=9-4, to=7-1]
\end{tikzcd}
\subsection*{Problema 3}
\begin{code}
squares = anaRose gsq
...
shrink ((x,y),s) = let ns = s/3 in ((x+ns,y+ns),ns)

divide :: Square -> [Square]
divide ((x,y),s) = [a,b,c,d,e,f,g,h]
    where
        ns = s/3
        x1 = x
        x2 = x+ns
...
gsq (sq, 0) = (shrink sq, [])
gsq (sq, n) = (shrink sq, zip (divide sq) (replicate 8 (n-1)))

rose2List = cataRose gr2l

gr2l = cons . (id >< concat)

carpets = anaList ((id -|- (split (curry sierpinski ((0,0),32)) id)).outNat)
...
cgene = either nil addOrSum

addOrSum (x,[]) = [x]
addOrSum ((a,b),((c,d):t)) | a == c    = (a, b+d):t
                           | otherwise = (c,d):addOrSum ((a,b),t)

\end{code}
Geração dos jogos da fase de grupos:
\begin{code}

pairup xs = [x |-> y | (x:ys) <- tails xs, y <- ys]

matchResult criteria (a,b) = let winner = criteria (a,b) in
                                givePoints (a,b) winner

givePoints :: Match -> Maybe Team -> [(Team, Int)]
givePoints (a,b) Nothing = [((a,1)),(b,1)]
givePoints (a,b) (Just x) | a == x = [(a,3),(b,0)]
                          | b == x = [(a,0),(b,3)]
...
pgroupWinners crit= (return . (best 2 . consolidate . concat)) .! (mmap (pmatchResult crit))

pgroupWinners' criteria matches = do
                                   x <- mmap (pmatchResult criteria) matches
                                   return ((best 2 . consolidate . concat) x)


pmatchResult criteria (a,b) = do
                            winner <- criteria (a,b)
                            return (givePoints (a,b) winner)
